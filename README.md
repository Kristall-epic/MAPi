...
Preview | The image that will appear on the menu along your map, 256x128 is recommended, any size works | `get_texture_info("prev_cool")`
...
Param | Description | Example
----- | ----------- | --------
MapID | The number returned from hangout_map_add() and position on the menu | hangout_coolness = `MAPi.hangout_map_add()` \| `MAPi.hangout_edit_skybox(hangout_coolness)`|
Area | The area index of the skybox you wanna change, normally 1 | 1 \| 2 \| 4
Skybox | A table with the images, in skytype, specify "box" to use 6 images or "ico" to use only 1 image (the one specified as front) | { skytype = "box", up = skybox_up, down = skybox_dn, front = skybox_ft, back = skybox_bk, left = skybox_lf, right = skybox_rt}


### MAPi.hangout_add_env_tint()
...
### MAPi.get_cur_hangout()
Returns the current or last MapID of a hangout you have been in


### MAPi.get_levelnum_from_hangout()
Returns the levelnum of a MapID
Param | Description | Example
----- | ----------- | --------
MapID | The number returned from hangout_map_add() and position on the menu | hangout_coolness = `MAPi.hangout_map_add()` \| `MAPi.get_levenum_from_hangout(hangout_coolness)`|


### MAPi.get_hangout_from_levelnum()
...
### MAPi.menu_get_cur_selected()
Returns the selected MapID on the menu


### MAPi.check_players_in_hangout()
Returns the total number of players that are currently in the given mapID
Param | Description | Example
----- | ----------- | --------
MapID | The number returned from hangout_map_add() and position on the menu | hangout_coolness = `MAPi.hangout_map_add()` \| `MAPi.check_players_in_hangout(hangout_coolness)`|


### MAPi.check_players_hangout_per_act()
Returns a table with the number of players that are currently in each act of the given mapID
Param | Description | Example
----- | ----------- | --------
MapID | The number returned from hangout_map_add() and position on the menu | hangout_coolness = `MAPi.hangout_map_add()` \| `MAPi.check_players_hangout_per_act(hangout_coolness)`|


### MAPi.hangout_edit_bgm()
...
Param | Description | Example
----- | ----------- | --------
MapID | The number returned from hangout_map_add() and position on the menu | hangout_coolness = `MAPi.hangout_map_add()` \| `MAPi.hangout_edit_bgm(hangout_coolness, 1, my_very_cool_bgm)`|
...
### MAPi.hangout_add_entry_sound()
Adds a sound when the player warps into a hangout
Param | Description | Example
----- | ----------- | --------
MapID | The number returned from hangout_map_add() and position on the menu | hangout_coolness = `MAPi.hangout_map_add()` \| `MAPi.hangout_add_entry_sound(hangout_coolness)`|
...
Warps you to levelnum "dest" if you warp to "code" while in the given hangout, makes warps across hangouts not require prefixing the level to a certain number, also all hangouts have 64 to warp you to its levelnum, allowing for stuff like area warps
Param | Description | Example
----- | ----------- | --------
